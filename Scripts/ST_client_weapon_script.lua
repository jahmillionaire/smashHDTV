...
function target_impacted(weapon, weaponInteraction)
	...
	if Object.IsValid(target) and target:IsA("StaticMesh") and target.name == "enemy" then
	   ...
	   UI.ShowFlyUpText(tostring(damage), target:GetWorldPosition() + target:GetWorldTransform():GetUpVector() * Vector3.New(math.random(-50,50), math.random(-50,50), math.random(-50,50)), {duration = 0.1, color=Color.ORANGE, isBig=false})
